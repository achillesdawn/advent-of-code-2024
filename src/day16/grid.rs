...
impl Grid {
    pub fn new(s: String) -> Self {
        ...
        let (pos, end_pos) = Grid::find_start_pos(cols, rows, &grid);
    ...
    fn find_start_pos(cols: usize, rows: usize, grid: &Vec<Vec<char>>) -> (Vector, Vector){
        ...
        for y in 0..cols {
            for x in 0..rows {
                ...
                if c == 'E' {
                    end_pos = Some(Vector::new(x, y));
                } else if c == 'S' {
                    start_pos = Some(Vector::new(x, y));
                }
            }
        }
